...
const (
	...
	replicaGroupsID        = "replica-groups"
	servicesID             = "services"
	hostsID                = "hosts"
	weaveID                = "weave"
...
func updateKubeFilters(rpt report.Report, topologies []APITopologyDesc) []APITopologyDesc {
	...
	topologies = append([]APITopologyDesc{}, topologies...) // Make a copy so we can make changes safely
	for i, t := range topologies {
		if t.id == containersID || t.id == podsID || t.id == servicesID || t.id == deploymentsID || t.id == replicaSetsID || t.id == daemonsetsID || t.id == replicaGroupsID {
			topologies[i] = mergeTopologyFilters(t, []APITopologyOptionGroup{
...
func MakeRegistry() *Registry {
	...
	k8sReplicaGroupTypeFilter := APITopologyOptionGroup{
		ID:         "grouptype",
		Default:    "",
		SelectType: "union",
		NoneLabel:  "All Types",
	...
	registry.Add(
		...
		APITopologyDesc{
			id:          replicaGroupsID,
			parent:      podsID,
			renderer:    render.ReplicaGroupRenderer,
			Name:        "replica groups",
			Options:     []APITopologyOptionGroup{unmanagedFilter, k8sReplicaGroupTypeFilter},
			HideIfEmpty: true,
		},
		APITopologyDesc{
			id:          servicesID,
